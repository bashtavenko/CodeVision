    ...
    public class DatabaseObjectGraphCollector
    {
        ...
        public void CollectDependencies(List<string> databaseNames)
        {
            ...
            foreach (var databaseName in databaseNames)
            {
                try
                {
                    _logger.Log($"Collecting {databaseName} dependencies...");
        ...
        private void CollectDatabase(string databaseName)
        {
            ...
            foreach (var table in db.Tables)
            {
                ...
                foreach (var dependentTable in table.DependentTables)
                {
                    _graph.AddDependency(tableObject, new DatabaseObject(DatabaseObjectType.Table, dependentTable.FullyQualifiedName));
                }
            }
